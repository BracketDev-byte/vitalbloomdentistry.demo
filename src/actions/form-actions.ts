...
export default async function getFormData(
  ...
  try {
    ...
    const formData = {
      ...
      preferredContactMethod: rawFormData.get('preferredContactMethod') || 'email',
    ...
    const { firstName, lastName, email, phone, service, message, urgencyLevel, preferredContactMethod } = result.data;
    ...
    return {
      errors: {},
      success: true,
      message: 'Your appointment request has been sent securely. We will contact you within 24 hours.',
    };
  } catch (err: unknown) {
    ...
    return {
      ...
      success: false,
      message: 'We apologize, but there was an error processing your request. Please try again or call us directly.',
    };
  }
}
